...
class LMDistribution(BaseDistribution):
    ...
    def __init__(self,
        ...
        ----------
        max_new_tokens: int
            number of the maximum tokens in the generate tokens
        min_new_tokens: int
            number of the minimum tokens in the generate tokens  
        length_penalty: float
            length penalty for generate samples
        """
        ...
        # in the naver's original version, they fixed the logic of ''eos_token_id -> pad_token_id''
        # so, be careful about the pad token id, which is always different in different model.
        # following are cases used in our experiments.
        if 'en-zh' in tokenizer or 'en_zh' in tokenizer:
            self.tokenizer.pad_token = self.tokenizer.pad_token
            self.network = klass.from_pretrained(
                network,
                pad_token_id=self.tokenizer.pad_token_id
            )
        elif 'zh-en' in tokenizer or 'zh_en' in tokenizer:
            self.tokenizer.pad_token = self.tokenizer.pad_token
            self.network = klass.from_pretrained(
                network,
                pad_token_id=self.tokenizer.pad_token_id
            )
        elif 'en-de' in tokenizer or 'en_de' in tokenizer:
            self.tokenizer.pad_token = self.tokenizer.pad_token
            self.network = klass.from_pretrained(
                network,
                pad_token_id=self.tokenizer.pad_token_id
            )
        elif 'de-en' in tokenizer or 'de_en' in tokenizer:
            self.tokenizer.pad_token = self.tokenizer.pad_token
            self.network = klass.from_pretrained(
                network,
                pad_token_id=self.tokenizer.pad_token_id
    ...
    def save_pretrained(self, path: os.PathLike):
    ...
    def sample(self, context="", sampling_size=32, sum=True):
        ...
        return (
                [TextSample(ots, self.tokenizer.decode(ots, skip_special_tokens=self.skip_special_tokens)) for ots in output_tokens],
                seq_logprobs
            )
