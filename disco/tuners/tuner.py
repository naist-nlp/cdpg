...
from disco.utils.tqdm_handler import TqdmLoggingHandler
...
class Tuner():
    ...
    def __init__(self,
                 ...
                 target,
                 proposal=None,
                 context_distribution=SingleContextDistribution(),
                 loss=KLLoss(),
                 features=[], 
                 track_metrics=["kl", "tv", "js"],
                 track_divergence_from_base=False,
                 save_checkpoint_every=10, #追記部分
                 save_dir='.', #追記部分
                 dynamic_mode=False,
                 domain="",
                 experiment_id="",
        ...
        ----------
        save_checkpoint_every: int (default: 10)
            save_checkpoint per setting times
        ...
        self.base_bleu=base_bleu
        self.base_f1 = base_f1
        self.threshold_update_num = threshold_update_num
        self.max_epoch = max_epoch
        self.src_lang = src_lang
        self.tgt_lang = tgt_lang
        self.log_file_path = log_file_path
        self.supervising_file_path_dict = supervising_file_path_dict
    ...
    def _compute_gradient(self, samples, proposal_log_scores, target_log_scores, model_log_scores, context, n_steps):
        ...
        if z_value > 0:
            ...
            loss.backward()
        #else:
        #    logger.warn(f'z_value is 0. Therefore, the loss will not be updated.')
    ...
    def _step(self):
        ...
        for context in tqdm(contexts, desc='Sampling and Scoring', leave=False):
            ...
            for s in trange(n_steps, desc='Update and Compute Gradient', leave=False):
    ...
    def tune(self):
        ...
        if self.dynamic_mode:
            ...
            file.write(f"logging: current phase is {self.current_phase} \n")
            file.write(f"logging: base bleu score is {self.base_bleu}, base f1 score is {self.base_f1} \n")
            file.write(f"logging: threshold update number for acception is {self.threshold_update_num} \n")
            file.close()
        with trange(self.params["n_gradient_steps"], desc='fine-tuning') as t:
            for s in t:
                ...
                if  0 == (s + 1) % self.save_checkpoint_every:
                    current_epoch = s + 1
                    if self.dynamic_mode:
                        file = open(self.log_file_path, "a", encoding='utf-8')
                        current_bleu, current_f1 = get_current_dev(self.src_lang, self.tgt_lang, self.model.tokenizer, self.model.network, self.domain, self.model.device, self.experiment_id, self.supervising_file_path_dict)
                        file.write(f"logging: current epoch is {current_epoch} \n")
                        ...
                        if current_bleu > best_bleu and current_bleu > self.base_bleu:
                            ...
                            update_num_bleu = update_num_bleu + 1
                            file.write(f"logging: updating best bleu, current update number is {update_num_bleu} \n")
                        ...
                        file.close()
                    self.model.save_pretrained(os.path.join(self.save_dir, f"{s+1}-epoch"))
...
def translate(example, model, device, tokenizer):
...
def get_current_dev(src_lang, tgt_lang, tokenizer, model, domain, device, experiment_id, supervising_file_path_dict):
    model.eval()
    src_path, tgt_path = supervising_file_path_dict["src_path"], supervising_file_path_dict["tgt_path"]
    srcs, refs = _read_text_(src_path), _read_text_(tgt_path)
    tranlation_dataset = Dataset.from_dict({"src": srcs, "tgt": refs})
    tokenized_dataset = tranlation_dataset.map( lambda samples: tokenizer([sample for sample in samples['src']],
    ...
    translate_with_model = partial(translate, model=model, device=device, tokenizer=tokenizer)
    ...
    return round(float(bleu["score"]), 6), round(float(f1), 6)